...
# find sensors with adequate data (more than 2 years) that havent been removed or is inactive
...
#sensor_model_nb <- function(df) {
#  glm.nb(Hourly_Counts ~ Day + Time, data = df)
#}

#sensor_model_nbwm <- function(df) {
#  glm.nb(Hourly_Counts ~ Day + Time + Month, data = df)
#}

#sensor_model_nbcwm <- function(df) {
#  glm.nb(Hourly_Counts ~ Day + cos(pi*Time/12) + sin(pi*Time/12) + Month, data = df)
#}

# test multiple models for each row of the dataframe
#by_sensor_id <- by_sensor_id %>% 
#  mutate(model = map(data, sensor_model_nb)) %>%
#  mutate(aic = map(model, AIC)) %>%
#  mutate(model2 = map(data, sensor_model_nbc)) %>%
#  mutate(aic2 = map(model2, AIC)) %>%
#  mutate(choice = ifelse(as.numeric(aic) < as.numeric(aic2), "one", "two"))%>%
#  arrange(choice)



# best model!
sensor_model_nbc <- function(df) {
  glm.nb(Hourly_Counts ~ Day + cos(pi*Time/12) + sin(pi*Time/12), data = df)
}

# create our model and add it to each row of the dataframe
by_sensor_id <- by_sensor_id %>% 
  mutate(model = map(data, sensor_model_nbc)) %>%
  mutate(glance = map(model, broom::glance))

#check one of our models
by_sensor_id$model[[1]]

##### use sensor_model_nbc for all sensors
...
add_all_predictions <- function(mod){
  ...
  to_pred <- tidyr::crossing(Day, Time)
...
test <- by_sensor_id$data[[1]]

by_sensor_id$Sensor_ID
...
#  unnest(glance) %>%
#  mutate(model2 = map(data, sensor_model_nbc)) %>%
#  mutate(glance2 = map(model2, broom::glance)) %>%
#  unnest(glance2)
